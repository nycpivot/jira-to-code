...
class GrokProvider(LLMProvider):
  ...
  def _endpoint(self) -> str:
    base = self.config.base_url or "https://api.x.ai/v1"
  ...
  def generate(self, request_timeout: int = 500, **params) -> Generation:
    ...
    if "max_tokens" in params:
      payload["max_new_tokens"] = int(params["max_tokens"])
    ...
    try:
      ...
      raise RuntimeError(f"xAI error {resp.status_code}: {resp.text}") from None
